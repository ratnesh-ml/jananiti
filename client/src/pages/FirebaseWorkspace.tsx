...
import CivicDesktopRail from "@/components/CivicDesktopRail";
import CivicLifecycleTimeline from "@/components/CivicLifecycleTimeline";
import { CivicFeedSkeleton } from "@/components/CivicWorkspaceSkeleton";
import CivicWorkspaceNav, { type CivicWorkspaceScreen } from "@/components/CivicWorkspaceNav";
import { CivicActivityScreen, CivicDetailScreen, CivicExploreScreen, CivicProfileScreen, CivicWelcomePanel, type CivicWorkspaceRecord } from "@/components/CivicWorkspaceSupplement";
...
import { getFirebaseEnvironmentLabel, isFirebaseEvidenceUploadsEnabled, isFirebaseFreeStageConfigured } from "@/lib/firebase/config";
import { getTestOnlyCivicRecords, isPrimaryLaunchHost } from "@/lib/firebaseWorkspacePresentation";
import { calculateDemoDrfi, defaultDemoFactors, drfiFactorDefinitions, type DemoDrfiFactors } from "@/lib/browserLocalDemo";
import { Bell, CircleGauge, CircleUserRound, FilePlus2, LogOut, MapPin, MessageCircle, RefreshCcw, Search, Send, ShieldCheck, ThumbsDown, ThumbsUp, UsersRound } from "lucide-react";
...
export default function FirebaseWorkspace() {
  ...
  const evidenceUploadsEnabled = isFirebaseEvidenceUploadsEnabled();
  const visualPreview = import.meta.env.DEV && new URLSearchParams(window.location.search).get("preview") === "workspace";
  const launchMode = isPrimaryLaunchHost(window.location.hostname);
  ...
  const openSearch = () => {
    ...
    setNotice("Search accessible records by title, description, locality, category, or stored status.");
  };
  ...
  const createRecord = async () => {
    ...
    setIsSaving(true);
    try {
      if (file && !evidenceUploadsEnabled) {
        setError("Evidence bytes are unavailable in this no-cost workspace because Firebase Storage is not provisioned. You can submit the report without a file.");
        return;
      }
  ...
  if (isLoadingRecords && records.length === 0) return <main className="min-h-[100dvh] bg-[#f7f8fa] p-4 sm:p-6"><section className="mx-auto max-w-2xl"><p className="civic-route-enter mb-4 text-sm font-bold text-[#40505f]">Loading protected civic records…</p><CivicFeedSkeleton /></section></main>;

  return <main className="min-h-[100dvh] overflow-x-hidden bg-[#f7f8fa] pb-8 text-[#0a1317]"><section className="mx-auto max-w-6xl px-3 py-3 sm:px-6 sm:py-6 lg:px-8"><header className="overflow-hidden rounded-[26px] bg-[#0a1317] px-4 py-5 text-white shadow-[0_16px_42px_rgba(10,19,23,.15)] sm:rounded-[30px] sm:px-8 sm:py-7"><div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-[.17em] text-[#8bc5ff] sm:text-xs">{launchMode ? "JanaNiti · local civic network" : `JanaNiti · Firebase civic workspace${firebaseEnvironment !== "production" ? ` · ${firebaseEnvironment}` : ""}${visualPreview ? " · visual preview" : ""}`}</p><h1 className="mt-2 max-w-3xl text-3xl font-semibold tracking-[-.045em] sm:text-5xl">{launchMode ? "Make a complaint visible. Keep every step clear." : "A civic feed that keeps the real work visible."}</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-[#d0d7de]">{launchMode ? "Report a local issue privately or with your community, follow its progress, and help separate support from verification." : "Choose private reporting or a community post, keep reactions separate from truth verification, and follow the real complaint timeline."}</p></div>{user ? <button onClick={() => signOutFromFirebaseIfPresent()} className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 self-start rounded-full bg-white/10 px-4 text-sm font-bold transition hover:bg-white/15"><LogOut className="h-4 w-4" />Sign out</button> : <button onClick={signIn} className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 self-start rounded-full bg-white/10 px-4 text-sm font-bold transition hover:bg-white/15">Sign in</button>}</div></header>
    ...
    <CivicDesktopRail name={user?.displayName || user?.email || "Citizen"} accessibleRecordCount={records.length} onSelect={navigateWorkspace} onNotice={setNotice} />
    ...
    {!evidenceUploadsEnabled && <aside className="mt-3 rounded-2xl border border-[#c9dff4] bg-[#f4f9ff] px-4 py-3 text-sm leading-6 text-[#245477]"><strong>Attachments are not available yet.</strong> You can still submit a text-and-locality report. Never add identity documents, contact details, or sensitive files.</aside>}
    <section className="civic-route-enter mt-3 flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-[#e1e6eb] bg-white p-2 shadow-sm"><button onClick={() => { navigateWorkspace("explore"); setNotice("Explore shows records currently available to your account. Locality filtering will activate after a protected profile locality setting is published."); }} className="inline-flex min-h-10 items-center gap-2 rounded-xl px-3 text-xs font-bold text-[#40505f] hover:bg-[#f5f8fb]"><MapPin className="h-4 w-4 text-[#0064e0]" />Locality · Explore</button><div className="flex items-center gap-1"><button onClick={openSearch} aria-label="Search civic records" className="grid h-10 w-10 place-items-center rounded-xl text-[#40505f] hover:bg-[#f5f8fb]"><Search className="h-4 w-4" /></button><button onClick={() => navigateWorkspace("activity")} aria-label="Open updates" className="grid h-10 w-10 place-items-center rounded-xl text-[#40505f] hover:bg-[#f5f8fb]"><Bell className="h-4 w-4" /></button><button onClick={() => navigateWorkspace("profile")} aria-label="Open profile" className="grid h-10 w-10 place-items-center rounded-xl text-[#40505f] hover:bg-[#f5f8fb]"><CircleUserRound className="h-5 w-5" /></button></div></section>
    ...
    {activeScreen === "home" && visibleRecords.length === 0 && <CivicWelcomePanel onSelect={navigateWorkspace} onNotice={setNotice} />}
    ...
    <section className="mt-4 rounded-[24px] border border-[#e1e6eb] bg-white p-4 shadow-[0_1px_4px_rgba(20,22,26,.08)] sm:rounded-[28px] sm:p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-[#e8f2ff] text-[#0064e0]"><CircleGauge className="h-5 w-5" /></span><div><h2 className="font-bold">Community priority context</h2><p className="text-xs text-[#5d6c7b]">A transparent indicator to support protected coordinator review. Community reactions never change it.</p></div></div><div className="flex items-center gap-2"><span className="rounded-full border border-[#dce6f5] bg-[#f4f8ff] px-3 py-2 text-sm font-bold text-[#075eb7]">{priority.score}/100 · {priority.band}</span><button type="button" onClick={() => setPriorityPanelOpen((open) => !open)} aria-expanded={priorityPanelOpen} className="min-h-10 rounded-xl border border-[#dfe5eb] bg-white px-3 text-xs font-bold text-[#40505f] transition hover:border-[#0064e0] hover:text-[#0064e0]">{priorityPanelOpen ? "Hide inputs" : "Review inputs"}</button></div></div>{(!launchMode || priorityPanelOpen) ? <><div className="mt-5 grid gap-x-6 gap-y-4 md:grid-cols-2">{drfiFactorDefinitions.map((factor) => <label key={factor.key} className="grid gap-1.5"><span className="flex items-center justify-between text-xs font-bold text-[#40505f]"><span>{factor.label} <span className="font-normal text-[#718191]">× {factor.weight.toFixed(2)}</span></span><span>{factors[factor.key]}</span></span><input type="range" min="0" max="100" value={factors[factor.key]} onChange={(event) => setFactors({ ...factors, [factor.key]: Number(event.target.value) })} className="accent-[#0064e0]" /></label>)}</div><DrfiAdminRecommendation factors={factors} /></> : <p className="mt-4 rounded-2xl bg-[#f5f8fb] px-4 py-3 text-sm leading-6 text-[#5d6c7b]">Review the eight transparent inputs when you need to understand this indicator. Final civic prioritisation remains with protected human review.</p>}</section>
    <footer className="civic-route-enter mt-4 flex items-start gap-3 rounded-[22px] border border-[#d6eadc] bg-[#f2fbf4] p-4 text-sm leading-6 text-[#356a55]"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-[#31a24c]" /><p><strong>Privacy and safety boundary:</strong> Firestore rules restrict records by visibility and reporter identity. Public reports share only the wording and locality you choose—never enter Aadhaar, contact details, or exact home addresses. Evidence files remain disabled until Storage, retention, and billing are separately approved. AI suggestions are editable drafts; deterministic DRFI and protected human review remain the only priority path.</p></footer>
    ...
    <div id="workspace-explore">{activeScreen === "explore" && <CivicExploreScreen records={visibleRecords} onOpen={openRecord} onNotice={setNotice} searchFocusToken={searchFocusToken} />}</div>
